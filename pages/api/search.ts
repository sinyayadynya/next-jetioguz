...
export default async function handler(
  ...
  try {
    ...
    // We're going to search for articles or places.
    ...
    // 1. Search all places with the search keys.
    const places = await drupal.getResourceCollection("node--place", {
      locale: body.locale,
      defaultLocale: body.defaultLocale,
      params: new DrupalJsonApiParams()
        .addFields("node--place", ["title", "path", "created"])
